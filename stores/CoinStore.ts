...
export default class CoinStore {
					...
					if (balance.length > 0)
						balance.forEach((asset) => {
							...
							coins.push(
								new Coin(
									currentInfo,
									fromDenomToPrice(asset.denom, this.remoteConfigs.prices),
								),
							)
						})
					else {
	...
	async send(coin: SupportedCoins, address: string, dollar: number) {
		...
		if (
			!(
				this.walletStore.activeWallet &&
				this.walletStore.activeWallet.wallets[coin]
			)
		)
			return
		...
		try {
			...
			const data: FromToAmount = {
				...
				amount: fromDollarsToAmount(
					dollar,
					coinClass.coin.denom(),
					this.remoteConfigs.prices,
				),
			}
