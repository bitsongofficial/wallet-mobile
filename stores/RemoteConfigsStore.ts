	...
	async requestData() {
		...
		promises.push(
			new Promise((resolve, reject) => {
				setTimeout(() => {
					resolve([SupportedCoins.BITSONG])
				}, 1000)
			}),
		)
		...
		try {
			...
			const [enabledCoins, coingeckoPrices, pushNotificationToken] =
				results as [SupportedCoins[], CoingeckoPrices, string]
			runInAction(() => {
				for (const sc of Object.values(SupportedCoins)) {
					this.prices[sc] = coingeckoPrices[sc].usd ?? 1
				}
				// this.prices.bitsong = bitsongPrice
				this.enabledCoins.splice(
					1,
