	...
	})
	{
		...
		connector.on("call_request", async (error, payload) => {
			...
			for(const wallet of this.wallets)
			{
				...
				if(address == params.value.fromAddress) fromWallet = wallet as CosmosWallet
			}
			...
			if(params.typeUrl != "/cosmos.bank.v1beta1.MsgSend") data = {
				type: params.typeUrl,
				data: params.value,
			}			
